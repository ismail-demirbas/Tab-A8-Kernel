...
#ifndef struct_size
#define struct_size(p, member, n) 	(offsetof(typeof(*(p)), member) + sizeof(*(p)->member) * (n))
#endif
				      ...
				      bool backmost)
{
	...
	if (backmost) {
		...
		/*
		 * 4.14 backport note: upstream expands the readahead window
		 * via readahead_expand() here instead of eagerly reading
		 * extra pages. That API does not exist in 4.14 (list-based
		 * readpages has no notion of an expandable window), so both
		 * the readahead and sync-readpage cases now fall through to
		 * the eager erofs_grab_cache_page_nowait() loop below. This
		 * is a real behavior change: we lose the lazy-expand
		 * optimization, replaced by eager prefetch of the same
		 * trailing pages.
		 */
...
static int z_erofs_readahead(struct file *filp,
			     struct address_space *mapping,
			     ...
			     unsigned int nr_pages)
{
	...
	struct page *first_page = list_last_entry(pages, struct page, lru);
	gfp_t gfp = readahead_gfp_mask(mapping);
	const unsigned int orig_nr_pages = nr_pages;

	f.readahead = true;
	f.headoffset = (erofs_off_t)first_page->index << PAGE_SHIFT;

	/*
	 * 4.14 backport note: no readahead_control here, so we approximate
	 * the readahead span as [first_page->index, first_page->index +
	 * nr_pages) in PAGE_SIZE units, same span the caller already
	 * committed to via the pages list.
	 */
	z_erofs_pcluster_readmore(&f, f.headoffset +
				  ((erofs_off_t)nr_pages << PAGE_SHIFT) - 1,
				  &pagepool, true);
	trace_erofs_readpages(mapping->host, first_page, nr_pages, false);
	...
	z_erofs_runqueue(inode->i_sb, &f, &pagepool,
			 ...
			 orig_nr_pages <= sbi->opt.max_sync_decompress_pages);
	if (f.map.mpage)
		put_page(f.map.mpage);
	erofs_release_pages(&pagepool);
	return 0;
